...
const createBlog = async (req, res) => {
    try {
        ...
        // Extract params
        const { title, body, authorId, category, subcategory, isPublished } = requestBody;
        ...
        let blogData = {
            ...
            published: isPublished ? new Date() : null
        }
...
const updateBlog = async (req, res) => {
    try {
        ...
        if (!isValidRequestBody(requestBody)) {
            res.status(401).send({ status: false, message: 'No parameters passed. Blog unmodified', data: blog })
            return
        }
...
const deleteBlogByParams = async (req, res) => {
    try {
